...
export async function GET(request: NextRequest) {
    ...
    // Calculate total votes for each candidate in each nomination
    const nominationsWithTotalVotes = nomination.map((nomination) => {
        const candidatesWithTotalVotes = nomination.candidates.map((candidate) => {
        ...
        const sortedCandidates = candidatesWithTotalVotes.sort((a, b) => b.totalVotes - a.totalVotes);
    ...
    const sortedNominations = nominationsWithTotalVotes.sort((a, b) => {
        const totalVotesA = a.candidates.reduce((total, candidate) => total + candidate.totalVotes, 0);
        const totalVotesB = b.candidates.reduce((total, candidate) => total + candidate.totalVotes, 0);
